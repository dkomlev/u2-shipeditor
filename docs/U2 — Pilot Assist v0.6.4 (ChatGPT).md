# U2 — Pilot Assist v0.6.4 (ChatGPT)
...
| ```javascript
slip_lim_deg
``` | float | 2…30° | Жёсткий лимит | 15° |
| ```javascript
beta_base
``` | float | −5…+5° | Базовый слип | 0° |
| ```javascript
k_input
``` | float | 0.5…1.5 | Вклад руля в β* | 1.0 |
| ```javascript
k_lead
``` | float | 0…2 | Опережение по ω_yaw | 0.6 |
| ```javascript
k_ff
``` | float | 0…2 | Feed‑forward по Δv_lat | 0.3 |
| ```javascript
kp_β
``` | float | 0.5…6 | P по e_β | 2.0 |
| ```javascript
kd_β
``` | float | 0…2 | D по e_β | 0.6 |
| ```javascript
kp_lat
``` | float | 0…6 | P по Δv_lat | 1.2 |
| ```javascript
kd_lat
``` | float | 0…2 | D по Δv_lat | 0.3 |
| ```javascript
cap_main_coupled
``` | float | 0.2…1.2 | Доля продол. капы | 0.9 |
| ```javascript
jerk_max
``` | float | ≥0 | Лимит рывка (m/s³) | 12 |
| ```javascript
tau_course
``` | float | 0.1…1.0 s | LPF по курсу | 0.35 s |
| ```javascript
tau_vlim
``` | float | 0.1…1.0 s | LPF лимитера | 0.4 s |
| ```javascript
w_steer()
``` | curve | [0..1]→[0.1..0.8] | Вес рулёжн. вектора | см. пресеты |
| ```javascript
f_cap()
``` | curve |    | Снижение капы при | eβ |
| ```javascript
oversteer_bias
``` | float | −0.1…0.1 | Лёгкое «пере» | +0.02 |
| ```javascript
k_omega
``` | float | 0…1 | Опережение по ω | 0.25 |
